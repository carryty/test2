...
class ControlledVehicle(Vehicle):
    ...
    def plan_route_to(self, destination: str) -> "ControlledVehicle":
        ...
        """
        try:
            path = self.road.network.shortest_path(self.lane_index[1], destination)   # 寻找从当前车道索引（self.lane_index[1]）到目的地的最短路径
        except KeyError:
            path = []
        if path:
            self.route = [self.lane_index] + [(path[i], path[i + 1], None) for i in range(len(path) - 1)]  # 列表的第一个元素是当前车道索引(self.lane_index),其余元素是路径中相邻节点的元组(path[i], path[i + 1], None)，其中None表示没有指定的动作
        else:
