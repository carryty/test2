...
class Vehicle(RoadObject):
    ...
    @classmethod
    def lane_create_random(cls, road: Road,
                        speed: float = None,
                        lane_from: list = None,
                        lane_to: Optional[str] = None,
                        lane_id: Optional[int] = None,
        ...
        _from = road.np_random.choice(lane_from)
        _to = lane_to or road.np_random.choice(list(road.network.graph[_from].keys()))
        _id = lane_id if lane_id is not None else road.np_random.choice(len(road.network.graph[_from][_to]))
        lane = road.network.get_lane((_from, _to, _id))
        if speed is None:
    ...
    def lane_type(self):
        if str(type(self.lane)) == "<class 'highway_env.road.lane.StraightLane'>":
            # print(str(type(self.lane)))
            # print("sucess")
            return 0
        elif str(type(self.lane)) == "<class 'highway_env.road.lane.CircularLane'>":
            return 1
        elif str(type(self.lane)) == "<class 'highway_env.road.lane.SineLane'>":
            return 2
        else:
            return None
    ...
    def lane_from(self):
        ...
        for _from, edges in graph.items():
            for _to, edge in edges.items():
                for _ in edge:
                    if self.lane == _:
                        return _from
