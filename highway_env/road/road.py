...
class RoadNetwork(object):
    ...
    def get_position(self, _start: str, _end: str, _number: int) -> np.array:
        """
        :param start: 开始位置索引
        :param end: 结束位置索引
        :param number: 道路id
        ...
        for _from, to_dict in self.graph.items():
            for _to, lanes in to_dict.items():
                for _id, l in enumerate(lanes):
                    if _from == _start and _to == _end and _id == _number:
                        lane = self.graph[_from][_to][_id]
                        return lane.start,lane.end
                    # else:
                    #     return "找不到相应道路"
        # tup1 = tuple[_start, _end, _number]
        # lane = RoadNetwork.get_lane(self, tup1)
        # return lane.start, lane.end
...
class Road(object):
    ...
    def close_objects_to(self, vehicle: 'kinematics.Vehicle', distance: float, count: Optional[int] = None,
        ...
        if sort:
            objects_ = sorted(objects_, key=lambda o: abs(vehicle.lane_distance_to(o)))    # 依据每个元素与vehicle之间的车道距离的绝对值来排序
        if count:
