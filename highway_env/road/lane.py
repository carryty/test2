...
class StraightLane(AbstractLane):
    ...
    def __init__(self,
        ...
        self.length = np.linalg.norm(self.end - self.start)     # 默认二范数，（x^2+y^2）^(1/2)
        self.line_types = line_types or [LineType.STRIPED, LineType.STRIPED]
        self.direction = (self.end - self.start) / self.length
        self.direction_lateral = np.array([-self.direction[1], self.direction[0]])   # 创建数组
...
class SineLane(StraightLane):
    ...
    def __init__(self,
                 ...
                 amplitude: float,   # 车道的振幅
                 pulsation: float,   # 车道的脉动频率
                 phase: float,       # 车道的初始相位
        ...
        """
        super().__init__(start, end,  width, line_types, forbidden, speed_limit, priority, type)
