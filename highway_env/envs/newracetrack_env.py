...
class NewRacetrackEnv(AbstractEnv):
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            "observation": {
                "type": "Kinematics",
                "absolute": True,
                "features": ["presence", "x", "y", "vx", "vy"],#, "Lane_type", "heading"
    ...
    def _create_road(self) -> None:
        ...
        speedlimits = [None, 30, 30, 30, 30, 30, 30, 30, 30]
    ...
    def _create_vehicles(self) -> None:
        ...
        if create_vehicles:
            self.road.vehicles.append(
                other_vehicles_type.lane_create_random(self.road, lane_from=["b"], speed=19))
            self.road.vehicles.append(
                other_vehicles_type.lane_create_random(self.road, lane_from=["d"], speed=18))
            self.road.vehicles.append(
                other_vehicles_type.lane_create_random(self.road, lane_from=["e"], speed=19))
            self.road.vehicles.append(
                other_vehicles_type.lane_create_random(self.road, lane_from=["f"], speed=18))
            self.road.vehicles.append(
                other_vehicles_type.lane_create_random(self.road, lane_from=["g"], speed=19))
            self.road.vehicles.append(
                other_vehicles_type.lane_create_random(self.road, lane_from=["h"], speed=18))
    ...
    def _reward(self, action: Action) -> float:
        ...
        # reward = reward - (self.time - self.t) * 0.3
        self.t = self.time
    ...
    def point_rewards(self) -> float:
        ...
        if -5<vehicle_x-b[0]<5 and -6<=vehicle_y-b[1]<=6 and self.Arrival[0]:
            self.Arrival[0]=0
            if self.config["show_point"]:
                print("经过b点")
            return self.config["b_reward"]
        elif -8<=vehicle_x-d[0]<=8 and -7<vehicle_y-d[1]<7 and self.Arrival[1]:
            self.Arrival[1] = 0
            if self.config["show_point"]:
                print("经过d点")
            return self.config["d_reward"]
        elif -6<vehicle_x-f[0]<6 and -6<vehicle_y-f[1]<6 and self.Arrival[2]:
            self.Arrival[2] = 0
            if self.config["show_point"]:
                print("经过f点")
            return self.config["f_reward"]
        elif -6<vehicle_x-h[0]<6 and -6<vehicle_y-h[1]<6 and self.Arrival[3]:
            self.Arrival[3] = 0
            if self.config["show_point"]:
                print("经过h点")
